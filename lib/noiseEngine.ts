  ...
  async toggle(id: LayerId, on: boolean) {
    ...
    if (on) {
      if (!layer) {
        ...
        // Sources are single-use once stopped, so a fresh toggle-on after a
        // toggle-off needs an entirely new chain (source -> filters -> gain
        // -> master). Keep the volume the user had set, drop everything else.
        const rebuilt = this.buildChain(ctx, id);
        rebuilt.volume = layer.volume;
        this.layers.set(id, rebuilt);
        layer = rebuilt;
      }
